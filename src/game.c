...
void menuInit()
{
	menuPrincipal.option1 = (SDL_Rect)
	{ 111, 168, 550, 84 };
	menuPrincipal.option2 = (SDL_Rect)
	{ 148, 260, 492, 89 };
	menuPrincipal.option3 = (SDL_Rect)
	{ 150, 362, 491, 78 };

	commencerUnePartie.option1 = (SDL_Rect)
	{ 163, 185, 450, 82 };
	commencerUnePartie.option2 = (SDL_Rect)
	{ 73, 284, 616, 74 };
	commencerUnePartie.retour = (SDL_Rect)
	{ 7, 496, 156, 62 };

	reglesJeu.retour = (SDL_Rect)
	{ 13, 494, 167, 64 };

	mode2Joueurs.option1 = (SDL_Rect)
	{ 300, 285, 182, 66 };
	mode2Joueurs.option2 = (SDL_Rect)
	{ 303, 376, 180, 68 };
	mode2Joueurs.retour = (SDL_Rect)
	{ 14, 495, 160, 59 };
}
...
void initPartie(Plateau *pl, SDL_Renderer *renderer)
{
	...
	pl->bas.rect = (SDL_Rect)
	{ 697, 224, 72, 75 };
	pl->l1_gauche.rect = (SDL_Rect)
	{ 534, 396, 70, 76 };
	pl->l1_milieu.rect = (SDL_Rect)
	{ 534, 225, 72, 69 };
	pl->l1_droite.rect = (SDL_Rect)
	{ 535, 51, 70, 75 };
	pl->l2_gauche.rect = (SDL_Rect)
	{ 366, 397, 69, 74 };
	pl->l2_milieu.rect = (SDL_Rect)
	{ 364, 225, 70, 72 };
	pl->l2_droite.rect = (SDL_Rect)
	{ 366, 52, 69, 73 };
	pl->l3_gauche.rect = (SDL_Rect)
	{ 196, 394, 69, 75 };
	pl->l3_milieu.rect = (SDL_Rect)
	{ 196, 225, 70, 74 };
	pl->l3_droite.rect = (SDL_Rect)
	{ 193, 53, 72, 74 };
	pl->main_joueur = (SDL_Rect)
	{ 278, 530, 282, 62 };
	pl->haut.rect = (SDL_Rect)
	{ 31, 224, 72, 74 };
	...
	pl->chien_1.position = &pl->haut;
	pl->chien_1.position_precedente = &pl->haut;
	pl->chien_2.position = &pl->l3_droite;
	pl->chien_2.position_precedente = &pl->l3_droite;
	pl->chien_3.position = &pl->l3_gauche;
	pl->chien_3.position_precedente = &pl->l3_gauche;
	pl->lievre.position = &pl->bas;
	pl->lievre.position_precedente = &pl->bas;
	pl->bas.occupant = 'l';
	pl->haut.occupant = 'c';
	pl->l3_droite.occupant = 'c';
	pl->l3_gauche.occupant = 'c';
	/*Definition des statuts (occupee ou pas) des differentes cases */
	pl->bas.estOccupee = 1;
	pl->haut.estOccupee = 1;
	pl->l3_droite.estOccupee = 1;
	pl->l3_gauche.estOccupee = 1;
	pl->l3_milieu.estOccupee = 0;
	pl->l1_milieu.estOccupee = 0;
	pl->l1_gauche.estOccupee = 0;
	pl->l1_droite.estOccupee = 0;
	pl->l2_milieu.estOccupee = 0;
	pl->l2_gauche.estOccupee = 0;
	pl->l2_droite.estOccupee = 0;
...
int verifDeplacement()
{
	int x1 = caseDepart->x;
	int y1 = caseDepart->y;
	int x2 = caseArrivee->x;
	int y2 = caseArrivee->y;
...
void clickHandlerOptions(SDL_Window *window, SDL_Renderer *renderer, SDL_Point p)
{
	...
	else if (strstr(currentFenetre, "gameBoard") != NULL)
	{
		...
		verifGagnant(renderer);
	}
}

void verifGagnant(SDL_Renderer *renderer)
{
	int fin = 0;
	char gagnant = 'n';
	// On verifie si les chiens ont gagnes
	if (pl.lievre.position->x == 0 && pl.lievre.position->y == -1)
	{
		if (pl.l1_gauche.occupant == 'c' && pl.l3_gauche.occupant == 'c' && pl.l2_milieu.occupant == 'c')
		{
			gagnant = 'c';
			fin = 1;
		}
	}
	else if (pl.lievre.position->x == 0 && pl.lievre.position->y == 1)
	{
		if (pl.l1_droite.occupant == 'c' && pl.l3_droite.occupant == 'c' && pl.l2_milieu.occupant == 'c')
		{
			gagnant = 'c';
			fin = 1;
		}
	}
	else if (pl.lievre.position->x == 2 && pl.lievre.position->y == 0)
	{
		if (pl.l1_milieu.occupant == 'c' && pl.l1_droite.occupant == 'c' && pl.l1_gauche.occupant == 'c')
		{
			gagnant = 'c';
			fin = 1;
		}
	}
	// On verifie si le lievre a gagne 
	else if (pl.lievre.position->x <= pl.chien_1.position->x && pl.lievre.position->x <= pl.chien_2.position->x &&
		pl.lievre.position->x <= pl.chien_3.position->x)
	{
		gagnant = 'l';
		fin = 1;
	}
	if (fin)
	{
		SDL_Delay(1000);
		if (gagnant == 'l')
			switchScreen(renderer, "images/imagesBMP/vainqueur_lievre.bmp", "*", 0);
		else
			switchScreen(renderer, "images/imagesBMP/vainqueur_chiens.bmp", "*", 0);

		switchScreen(renderer, "images/imagesBMP/menu.bmp", "menuPrincipal", 2500);
	}
}